...
H(P/2:P/2+2,Q/2:Q/2+2)=laplacian;
%中心化
H2=im2double(H);
H2=shift(H2,P,Q);
%DFT
...
for i=2:1:M+1
    for j=2:1:N+1
        C2(i,j) =B2(i-1,j+1)+B2(i,j+1)+B2(i+1,j+1)+B2(i-1,j-1)+B2(i,j-1)+B2(i+1,j-1)+B2(i-1,j)+B2(i+1,j)-8*B2(i,j);
    end
end
